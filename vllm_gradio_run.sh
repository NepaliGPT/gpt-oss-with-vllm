...
SIF_PATH="${SIF_PATH:-/home/$USER/gpt-oss-with-vllm/vllm-gptoss.sif}"
...
export VLLM_CACHE_ROOT="${VLLM_CACHE_ROOT:-/home/${USER}/gpt-oss-with-vllm/.vllm}"
export XDG_CACHE_HOME="${XDG_CACHE_HOME:-/home/${USER}/.gradio_cache}"
export TMPDIR="${TMPDIR:-/home/${USER}/tmp}"
...
# Paths & logs
#######################################
WORK_DIR="/home/$USER/gpt-oss-with-vllm"
...
echo "ssh -L localhost:${GRADIO_PORT}:${SERVER}:${GRADIO_PORT} -L localhost:${VLLM_PORT}:${SERVER}:${VLLM_PORT} ${USER}@hpc.tu.edu.np" > "$PORT_FWD_FILE"

#######################################
# Env / modules / Python selection
#######################################
if [ -f /etc/profile.d/modules.sh ]; then . /etc/profile.d/modules.sh; fi
module load gnu14/ cuda
...
if [ "$PY_VER_OK" != "1" ]; then
  ...
  if [ -x "/home/${USER}/miniconda3/envs/vllm-hpc/bin/python3" ]; then
    PYTHON_BIN="/home/${USER}/miniconda3/envs/vllm-hpc/bin/python3"
  fi
fi
